...
describe('Selection Sort Algorithm', () => {
  test('should sort an array of numbers in ascending order', () => {
    ...
    const steps = []
    for (const step of selectionSort(input)) {
      steps.push(step)
    }
  ...
  test('should yield steps with proper structure', () => {
    ...
    steps.forEach(step => {
      ...
      expect(step).toHaveProperty('currentIndex')
      expect(step).toHaveProperty('minIndex')
      expect(step).toHaveProperty('comparingIndex')
      expect(step).toHaveProperty('swapIndices')
      expect(step).toHaveProperty('doneIndices')
    ...
    const finalStep = steps[steps.length - 1]
    expect(finalStep.description).toBe('Array is fully sorted')
  })
})