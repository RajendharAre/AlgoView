...
describe('Insertion Sort Algorithm', () => {
  ...
  test('should yield steps with proper structure', () => {
    ...
    steps.forEach(step => {
      ...
      expect(step).toHaveProperty('keyIndex')
      expect(step).toHaveProperty('comparingIndex')
      expect(step).toHaveProperty('shiftingIndex')
      expect(step).toHaveProperty('placedIndex')
    ...
    const finalStep = steps[steps.length - 1]
    expect(finalStep.description).toBe('Insertion sort complete')
  })
})