...
export const codeExamplesService = {
  ...
  async getCodeExamplesByLanguage(language) {
    try {
      const q = query(collection(db, 'codeExamples'), where('language', '==', language))
      const snapshot = await getDocs(q)
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
  ...
  async getAllVideoCourses() {
    try {
      const snapshot = await getDocs(
        query(collection(db, 'videoCourses'), orderBy('createdAt', 'desc'))
      )
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
...
export const docsService = {
  ...
  async getDocsByCategory(category) {
    try {
      const q = query(collection(db, 'docs'), where('category', '==', category))
      const snapshot = await getDocs(q)
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
...
export const qaService = {
  async getAllQuestions() {
    try {
      const snapshot = await getDocs(query(collection(db, 'qaQuestions'), orderBy('views', 'desc')))
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      }))
