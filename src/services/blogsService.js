...
export const getBlogBySlug = async slug => {
  try {
    const q = query(blogsCollection, where('slug', '==', slug), where('status', '==', 'published'))
...
export const searchBlogs = async searchTerm => {
  try {
    ...
    if (error.code === 'failed-precondition') {
      ...
      try {
        const fallbackQ = query(blogsCollection, where('status', '==', 'published'))
        const snapshot = await getDocs(fallbackQ)
...
export const getUserBlogs = async userId => {
  try {
    const q = query(blogsCollection, where('authorId', '==', userId), orderBy('createdAt', 'desc'))
    const snapshot = await getDocs(q)
