      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="text-[11px] text-[var(--text-secondary,#8e8ea0)] flex items-center gap-2 truncate">
            ...
            <span className={`flex items-center gap-1 ${hasApiKey ? 'text-[var(--accent,#10a37f)]' : 'text-amber-400'}`}>
              <span className={`inline-block w-1.5 h-1.5 rounded-full ${hasApiKey ? 'bg-[var(--accent,#10a37f)]' : 'bg-[var(--text-secondary,#8e8ea0)]'}`} />
              {hasApiKey ? 'Online' : <><AlertCircle className="w-3 h-3" /> Default Responses</>}
            </span>
          </div>
        </div>
      </div>
