...
export default function TutorialCard({ tutorial }) {
  ...
  const renderStars = rating => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<FaStar key={`star-${i}`} style={{ color: COLORS.star }} />)
    }

    if (hasHalfStar) {
      stars.push(<FaStar key="half-star" style={{ color: COLORS.star, opacity: 0.5 }} />)
    }

    const emptyStars = 5 - Math.ceil(rating)
    for (let i = 0; i < emptyStars; i++) {
      stars.push(<FaStar key={`empty-${i}`} style={{ color: COLORS.border.light }} />)
    }
    ...
    <>
      ...
      >
        ...
        >
          {imgFailed || !imgSrc ? (
            ...
            >
              <span style={{ color: catColor.text }}>
                {tutorial.category === 'Web' ? (
                  <FaGlobe size={36} />
                ) : tutorial.category === 'DevOps' ? (
                  <FaCog size={36} />
                ) : tutorial.category === 'Cloud' ? (
                  <FaCloud size={36} />
                ) : tutorial.category === 'AI' ? (
                  <FaRobot size={36} />
                ) : (
                  <FaBook size={36} />
                )}
              </span>
              <span style={{ color: catColor.text, fontWeight: 600, fontSize: 13 }}>
                {tutorial.category}
              </span>
            </div>
          ) : (
          ...
          {tutorial.isPremium && (
            <div
              style={{ backgroundColor: PREMIUM_COLORS.bg, color: PREMIUM_COLORS.text }}
        ...
        <div className="p-5">
          ...
          <h3
            ...
            className="text-lg font-bold mb-2 line-clamp-2 cursor-pointer transition"
          >
            {tutorial.title}
          </h3>
          ...
          <div className="flex flex-wrap gap-1 mb-4">
            {tutorial.tags.slice(0, 2).map((tag, i) => (
              <span
                key={i}
                style={{ color: TAG_COLORS.text, backgroundColor: TAG_COLORS.bg }}
          ...
          <div
            ...
            className="flex items-center justify-between mb-4 pb-4 border-t"
          >
            <div className="flex items-center gap-2">
              <div className="flex gap-0.5">{renderStars(rating)}</div>
              <span style={{ color: COLORS.text.primary }} className="text-sm font-semibold">
                {rating}
              </span>
          ...
          {/* Author */}
          <div
            style={{ color: COLORS.text.tertiary, borderTopColor: COLORS.border.light }}
          ...
          {/* Read Button */}
          <button
            onClick={() => navigate(`/development/tutorials/${tutorial.id}`)}
            style={{ backgroundColor: COLORS.bg.dark, color: COLORS.text.light }}
            className="w-full mt-4 py-2 rounded-lg font-semibold hover:opacity-90 transition"
          >
            Start Learning
          </button>
        </div>
        ...
        >
          ...
          >
            ...
            <div
              ...
              className="sticky top-0 p-6 border-b flex justify-between items-start"
            >
              <div>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-4 gap-4 mb-6">
                <div style={{ backgroundColor: COLORS.bg.primary }} className="p-4 rounded-lg">
                  <div className="text-2xl mb-2">
                    <FaClock style={{ color: COLORS.text.tertiary }} />
                  </div>
                  <div style={{ color: COLORS.text.tertiary }} className="text-xs mt-1">
                    Duration
                  </div>
                ...
                <div style={{ backgroundColor: COLORS.bg.primary }} className="p-4 rounded-lg">
                  <div className="text-2xl mb-2">
                    <FaStar style={{ color: COLORS.star }} />
                  </div>
                  <div style={{ color: COLORS.text.tertiary }} className="text-xs mt-1">
                    Rating
                  </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {tutorial.tags.map((tag, i) => (
                    <span
                      key={i}
                      style={{ backgroundColor: TAG_COLORS.bg, color: TAG_COLORS.text }}
              ...
              <div style={{ backgroundColor: COLORS.bg.primary }} className="mb-6 p-4 rounded-lg">
                <div style={{ color: COLORS.text.tertiary }} className="text-sm mb-1">
                  Author
                </div>
                <div style={{ color: COLORS.text.primary }} className="text-lg font-bold">
                  {tutorial.author}
              ...
              <button
                onClick={() => {
                  ...
                  navigate(`/development/tutorials/${tutorial.id}`)
                }}
                style={{ backgroundColor: COLORS.bg.dark, color: COLORS.text.light }}
                className="w-full py-3 rounded-lg font-bold hover:opacity-90 transition text-lg"
              >
