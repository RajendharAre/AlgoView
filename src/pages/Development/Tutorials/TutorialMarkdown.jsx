...
export default function TutorialMarkdown({ content }) {
  ...
  const makeId = (text) => {
    const str = typeof text === 'string' ? text : React.Children.toArray(text).map(c => typeof c === 'string' ? c : c?.props?.children || '').join('');
  ...
  return (
    <div className="tutorial-markdown-content">
      <Markdown
        ...
        components={{
          h2({ children }) { return <h2 id={makeId(children)}>{children}</h2>; },
          h3({ children }) { return <h3 id={makeId(children)}>{children}</h3>; },
          pre({ children }) {
