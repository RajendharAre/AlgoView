...
const DFSVisualization = () => {
  ...
  const [callStack, setCallStack] = useState([]) // Visualizing the recursion stack
  ...
  const runAlgorithm = async () => {
    ...
    const gen = dfsInfo.generator(currentGraph, actualStartNodeId)
  ...
  return (
    <div className="flex flex-col-reverse md:flex-row min-h-screen md:h-screen bg-[#f8f9faff] text-[#212529] font-sans overflow-auto md:overflow-hidden">
      <CommonSidebar
        algorithmTitle="DFS Visualizer"
        algorithmSubtitle="Depth-First Search"
        isRunning={isRunning}
      ...
      >
        ...
        <div className="p-6 border-b border-[#f1f3f5] space-y-3">
          ...
          <div className="flex flex-col-reverse gap-1 min-h-[40px] p-2 bg-[#f8f9faff] rounded-xl border border-[#dee2e6]">
            {callStack.map((id, i) => (
              <div
                key={i}
                className={`px-2 py-1.5 text-white text-[10px] font-bold rounded-lg shadow-sm flex justify-between items-center animate-in slide-in-from-bottom-2 ${i === callStack.length - 1 ? 'bg-[#212529]' : 'bg-[#6c757d]'}`}
              >
                <span>Node {nodes.find(n => n.id === id)?.label}</span>
                {i === callStack.length - 1 && (
                  <span className="text-[7px] bg-white/20 px-1 rounded">TOP</span>
                )}
              </div>
            ))}
            {callStack.length === 0 && (
              <span className="text-[9px] text-[#adb5bd] italic p-1">Stack is empty</span>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 p-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <div className="p-3 bg-white border border-[#dee2e6] rounded-xl">
              <p className="text-[8px] font-black text-[#adb5bd] uppercase mb-1">Stack Depth</p>
              <p className="text-lg font-bold font-mono">{callStack.length}</p>
            </div>
          </div>
        </div>
      </CommonSidebar>
        ...
        <svg ref={svgRef} className="w-full h-full cursor-crosshair">
          ...
          {nodes.map(node => {
            ...
            if (isActive) {
              ...
              fill = COLORS.paleSlate
              stroke = COLORS.slateGrey
              textColor = COLORS.ironGrey
            }
            ...
            return (
              ...
              >
                <circle
                  ...
                  fill={fill}
                  stroke={stroke}
                  strokeWidth={isStart || isSelected || inStack ? '3' : '2'}
                  className="shadow-md"
                />
                <text
                  x={node.x}
...
const ToolButton = ({ active, onClick, disabled, icon, label, desc, danger }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    className={`w-full flex items-center gap-3 p-2.5 rounded-xl border transition-all text-left disabled:opacity-30 ${active ? 'bg-[#212529] border-[#212529] shadow-md translate-x-1' : 'bg-white border-transparent hover:bg-[#f8f9faff] hover:border-[#dee2e6]'}`}
  >
    <div
      className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${active ? 'bg-white/20 text-white' : danger ? 'bg-red-50 text-red-500' : 'bg-[#f1f3f5] text-[#6c757d]'}`}
    >
      {icon}
    </div>
