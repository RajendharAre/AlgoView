      ...
      >
        ...
        <div className="p-6 border-b border-[#f1f3f5] space-y-3">
          ...
          <div className="flex flex-col-reverse gap-1 min-h-[40px] p-2 bg-[#f8f9faff] rounded-xl border border-[#dee2e6]">
             {callStack.map((id, i) => (
               <div key={i} className={`px-2 py-1.5 text-white text-[10px] font-bold rounded-lg shadow-sm flex justify-between items-center animate-in slide-in-from-bottom-2 ${i === callStack.length - 1 ? 'bg-[#212529]' : 'bg-[#6c757d]'}`}>
                 <span>Node {nodes.find(n => n.id === id)?.label}</span>
                 {i === callStack.length - 1 && <span className="text-[7px] bg-white/20 px-1 rounded">TOP</span>}
               </div>
             ))}
             {callStack.length === 0 && <span className="text-[9px] text-[#adb5bd] italic p-1">Stack is empty</span>}
          </div>
        </div>
