...
const TSP = () => {
  ...
  const [status, setStatus] = useState('Depot: Node A. Click "Execute Solver" to find optimal route.');
  ...
  const solveTSP = async () => {
    console.log('🔧 solveTSP called - isRunning:', isRunning, 'nodes:', nodes.length, 'startNodeId:', startNodeId);
    ...
    const backtrack = async (currId, visitedSet, path, cost) => {
      ...
      setCurrentPath([...path]);
      console.log(`📍 Visiting: node=${currId}, path=${path.join(',')}  visited=${visitedSet.size}/${currentNodesCount}`);
      ...
      for (let nextNode of nodes) {
        if (!visitedSet.has(nextNode.id)) {
          ...
          if (cost + dist >= localMinCost) {
            console.log(`⏭️  Pruned: edge ${currId}→${nextNode.id}, cost ${cost + dist} >= ${localMinCost}`);
            continue;
          }
