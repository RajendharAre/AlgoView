...
export const graphColoringInfo = {
  ...
  difficulty: 'Medium',
  description:
    'Greedy algorithm for assigning colors to graph vertices such that no two adjacent vertices share the same color.',
  ...
  timeComplexity: {
    ...
    worst: 'O(V + E)',
    explanation:
      'Iterate through V vertices once: O(V). For each vertex, check neighbors: O(degree). Sum of all degrees = 2E. Find available color: O(chromatic number). Total: O(V + E).',
  },

  spaceComplexity: {
    value: 'O(V)',
    explanation:
      'Stores color assignment for each vertex in a hashmap/array and possibly a frequency map of used colors.',
  },
