...
const KruskalCanvas = React.forwardRef(
  (
    ...
    const renderEdgeStroke = useCallback(
      edgeIndex => {
    ...
    return (
      <svg ref={ref} className="w-full h-full cursor-crosshair" onClick={onCanvasClick}>
        ...
        {nodes.map(node => {
          ...
          const isActive =
            sortedEdges[currentEdgeIdx]?.u === node.id || sortedEdges[currentEdgeIdx]?.v === node.id
