...
const DijkstraVisualization = () => {
  ...
  const [currentStep, setCurrentStep] = useState(
    'Dijkstra finds the cheapest path using edge weights.'
  )
  ...
  const showPathTo = targetId => {
    ...
    setCurrentStep(
      `Shortest path to Node ${label} highlighted. Total Weight: ${distances[targetId]}`
    )
  }
  ...
  useEffect(() => {
    if (unreachableNodes.length > 0 && !isRunning) {
      ...
      setCurrentStep(
        `Unreachable nodes: ${unreachableLabels}. Connect them to the main graph to include in traversal.`
      )
    }
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 px-8 py-4 bg-white/90 backdrop-blur-sm border border-[#dee2e6] rounded-3xl shadow-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-black text-[#adb5bd] uppercase tracking-wide">
              Dijkstra's Algorithm: Arrows show edge direction. Connect isolated nodes to include in
              traversal.
            </span>
          </div>
        </div>
