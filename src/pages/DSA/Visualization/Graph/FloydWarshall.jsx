              ...
              <table className="w-full text-center text-[9px] border-collapse">
                <thead className="bg-[#f8f9faff] border-b border-[#dee2e6]">
                  <tr>
                    ...
                    {nodes.map(n => (
                      <th
                        key={n.id}
                        className={`p-2 font-black border-r border-[#dee2e6] ${currentJ === n.id ? 'bg-[#212529] text-white' : ''}`}
                      >
                        {n.label}
                      </th>
                    ))}
                ...
                <tbody>
                  {matrix.map((row, i) => (
                    <tr
                      key={i}
                      className={`border-b border-[#f1f3f5] ${currentI === i ? 'bg-[#e9ecefff]' : ''}`}
                    >
                      <td
                        className={`p-2 font-black border-r border-[#dee2e6] ${currentI === i ? 'bg-[#212529] text-white' : 'bg-[#f8f9faff]'}`}
                      >
                        {nodes[i]?.label}
                      </td>
                      {row.map((val, j) => (
                        <td
                          key={j}
                          className={`p-2 font-mono border-r border-[#f1f3f5] ${currentI === i && currentJ === j ? 'bg-[#6c757d] text-white font-bold' : currentK === j || currentK === i ? 'bg-[#f1f3f5]' : ''}`}
                        >
                          {val === Infinity ? '∞' : val}
                        </td>
                      ))}
      ...
      >
        <GraphCanvas
          ...
          highlightEdges={edges.reduce((acc, edge, idx) => {
            const isActive =
              (currentI === edge.u && currentJ === edge.v) ||
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 px-8 py-4 bg-white/90 backdrop-blur-sm border border-[#dee2e6] rounded-3xl shadow-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-black text-[#adb5bd] uppercase tracking-wide">
              Floyd-Warshall: Iteratively update distances between all pairs using intermediate node
              K.
            </span>
          </div>
        </div>
