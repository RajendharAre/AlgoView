...
const ClimbingStairs = () => {
  ...
  const runAlgorithm = async () => {
    ...
    for (let i = 2; i <= n; i++) {
      ...
      setDp([...localDp])
      setStatus(`Step ${i} = ${localDp[i - 1]} + ${localDp[i - 2]} = ${localDp[i]} ways.`)
      await sleep()
    }
            ...
            {currentIndex >= 2 && isRunning && (
              <g className="animate-in fade-in duration-500">
                ...
                <path
                  d={(() => {
                    const startX =
                      (currentIndex - 1) * (PILLAR_WIDTH + PILLAR_GAP) + PILLAR_WIDTH / 2
                    const startY = 600 - currentIndex * STEP_HEIGHT_UNIT - 60
                    const endX = currentIndex * (PILLAR_WIDTH + PILLAR_GAP) + PILLAR_WIDTH / 2
                ...
                <path
                  d={(() => {
                    const startX =
                      (currentIndex - 2) * (PILLAR_WIDTH + PILLAR_GAP) + PILLAR_WIDTH / 2
