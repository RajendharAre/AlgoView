...
// Convert PROBLEM_CATEGORIES to array format for CategoryFilter
const getCategoryArray = () => {
  return Object.entries(PROBLEM_CATEGORIES).map(([key, value]) => ({
    id: key,
    name: value.name,
  }))
}

// Legacy DSA_CATEGORIES for compatibility - DEPRECATED
  ...
  queue: {
    ...
    problems: [
      ...
      },
      {
        ...
        leetcodeSlug: 'design-circular-queue',
        completed: false,
      },
    ],
  },
    ...
    problems: [
      {
        ...
        difficulty: 'Medium',
        acceptance: 59.4,
        leetcodeUrl: 'https://leetcode.com/problems/letter-combinations-of-phone-number/',
        leetcodeSlug: 'letter-combinations-of-phone-number',
        completed: false,
      },
      {
        id: 'n-queens',
...
const Practice = () => {
  ...
  const verifyProblem = async (problem, isAutoCheck = false) => {
    ...
    try {
      ...
      const isSolved = await validateProblemSolved(leetcodeUsername, slugToCheck, session.clickedAt)
      ...
      if (!isSolved) {
        console.log(
          'Primary validation failed, trying fallback check for any accepted submission...'
        )
        try {
          ...
          console.log(
            'All submission slugs:',
            allSubmissions.map(s => s.titleSlug)
          )

          // Check for exact match first
          // Handle both string and numeric status codes
          const exactMatch = allSubmissions.find(
            submission =>
          ...
          if (exactMatch) {
            ...
            const partialMatches = allSubmissions.filter(
              submission => submission.titleSlug && submission.titleSlug.includes(slugToCheck)
            )
  ...
  const calculateOverallProgress = () => {
    ...
    const completedCount = allProblems.filter(problem => completedProblems.has(problem.id)).length
              ...
              {sortedProblems.map(problem => (
                ...
                >
                  ...
                  <div className="col-span-2">
                    <span
                      className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${DIFFICULTY_COLORS[problem.difficulty]}`}
                    >
                      {problem.difficulty}
                    </span>
                  </div>
                  ...
                  <div className="col-span-1 flex justify-end gap-1">
                    ...
                    >
                      <Star
                        className="h-4 w-4"
                        fill={favorites.has(problem.id) ? 'currentColor' : 'none'}
                      />
                    </button>
                  </div>
                </div>
