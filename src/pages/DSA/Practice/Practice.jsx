...
const Practice = () => {
      ...
      try {
        ...
        if (!solved) {
          try {
            ...
            solved = subs.some(
              s => s.titleSlug === slug && (s.status === 'Accepted' || s.status === 10)
            )
          } catch {
            solved = false
          }
        ...
        if (solved) {
          ...
          Promise.all([
            setDoc(
              ...
              {
                completedProblems: { ...data.completedProblems, [problem.id]: true },
                favorites: favObj,
              },
              { merge: true }
            ),
    ...
    problem => {
      ...
      if (!leetcodeUsername) {
        showPopup(
          ...
          'Please add your LeetCode username in your profile settings.',
          '',
          6000
        )
        return
      ...
      setDoc(
        doc(db, 'users', user.uid),
        { sessions: { [problem.id]: sessionData } },
        { merge: true }
  ...
  useEffect(() => {
    const handleFocus = async () => {
      ...
      focusTimerRef.current = setTimeout(async () => {
        ...
        if (isSolved) {
          console.log(
            `[FOCUS] ✅ Detected accepted solution for: ${problem.title} — marking complete`
          )
          const snap = await getDoc(doc(db, 'users', user.uid))
          const data = snap.exists() ? snap.data() : {}
          const favObj = {}
      ...
      if (snap.exists()) {
        ...
        if (d.completedProblems) {
          setCompletedProblems(
            new Set(Object.keys(d.completedProblems).filter(id => d.completedProblems[id]))
          )
        }
        if (d.favorites) {
        ...
        try {
          await setDoc(
            doc(db, 'users', user.uid),
            { completedProblems: {}, favorites: {}, sessions: {} },
            { merge: true }
          )
        } catch (e) {
  ...
  useEffect(() => {
    ...
    window.POPUP_TESTER = {
      trigger: problemId => {
        ...
        console.log(
          `✅ Popup triggered for "${p.title}" in ${(performance.now() - t0).toFixed(2)}ms`
        )
      },
      ...
      stats: () => {
        ...
        const result = {
          ...
          currentProblem: currentProblemRef.current
            ? { id: currentProblemRef.current.id, title: currentProblemRef.current.title }
            : null,
      ...
      testSwitch: (idA, idB) => {
        ...
        if (!pA || !pB) {
          console.error('❌ Problem IDs not found')
          return
        }
  ...
  const filteredProblems = activeCategory
    ? (DSA_CATEGORIES[activeCategory]?.problems || []).filter(p =>
  ...
  const toggleFavorite = id => {
    setFavorites(prev => {
      ...
      if (user) {
        ...
        setDoc(doc(db, 'users', user.uid), { favorites: favObj }, { merge: true }).catch(err =>
