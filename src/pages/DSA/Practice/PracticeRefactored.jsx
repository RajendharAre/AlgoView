...
const PracticeRefactored = () => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'users', user.uid), async docSnap => {
      if (docSnap.exists()) {
        ...
        if (userData.favorites) {
          setFavoriteProblems(
            new Set(
              Object.keys(userData.favorites).filter(problemId => userData.favorites[problemId])
            )
          )
        }
      } else {
  ...
  const filteredProblems = useMemo(() => {
    ...
    if (searchQuery) {
      problems = problems.filter(p => p.title.toLowerCase().includes(searchQuery.toLowerCase()))
    }
